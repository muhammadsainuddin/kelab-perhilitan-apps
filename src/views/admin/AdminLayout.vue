  ...
  <div class="flex h-screen overflow-hidden bg-gray-50 font-sans">
            ...
            <router-link v-for="item in group.items" :key="item.name"
              ...
              <button @click="navigate" :title="!sidebarOpen ? item.label : ''"
                ...
                <span :class="[
                  ...
                  <svg v-else-if="item.icon==='chat'" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
                </span>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <header class="bg-white border-b border-gray-200 px-4 md:px-8 py-3.5 flex items-center justify-between shadow-sm shrink-0 z-20">
        ...
        <div class="flex items-center gap-4">
          <!-- Butang kembali ke view ahli — hanya pada mobile -->
          <router-link to="/dashboard/utama"
            class="md:hidden flex items-center gap-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-xl px-2.5 py-1.5 text-xs font-bold transition-colors">
...
const pageTitles = {
  ...
  '/admin/sokongan':   { title: 'Tiket Sokongan',       sub: 'Urus pertanyaan dan aduan ahli' },
...
const menuGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'sokongan',  to: '/admin/sokongan',  label: 'Tiket Sokongan',   icon: 'chat' },
    ]
  },
  {
    title: 'Sistem',
