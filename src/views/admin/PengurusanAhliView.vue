  ...
  <div class="space-y-6 font-sans text-gray-900 pb-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-gray-500 text-sm mt-1">Urus data ahli, pendaftaran pukal (bulk), dan jejak profil 360°.</p>
      </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showEdit" class="fixed inset-0 z-[120] bg-black/50 backdrop-blur-sm flex items-center justify-center px-4" @click.self="showEdit = false">
          <div class="bg-white border border-gray-200 rounded-[24px] p-7 w-full max-w-md shadow-2xl">
            <div class="flex justify-between items-start mb-5 border-b border-gray-100 pb-4">
              <div>
                <h3 class="text-gray-900 font-black text-lg">Kemaskini Profil & Sistem</h3>
                <p class="text-gray-500 text-xs mt-1 font-bold">{{ formEdit.nama_pegawai }}</p>
              </div>
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select v-model="formEdit.status_ahli" class="w-full bg-gray-50 border border-gray-200 text-gray-900 font-bold rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#0F4C3A] transition-all">
                ...
                <div>
                  ...
                  <select v-model="formEdit.role" class="w-full bg-gray-50 border border-gray-200 text-gray-900 font-bold rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#0F4C3A] transition-all">
...
import ProfilAhli360 from '../../components/admin/ProfilAhli360.vue'; // Komponen modular baharu
...
// Fungsi Edit Dipanggil dari Table atau dari dalam Modal Profil 360
const bukaEdit = (ahli) => {
  formEdit.value = { no_kp: ahli.no_kp, nama_pegawai: ahli.nama_pegawai, no_ahli: ahli.no_ahli || '', status_ahli: (ahli.status_ahli || 'tidak aktif').toLowerCase(), role: ahli.role || 'Ahli' };
  showEdit.value = true;
};

const bukaEditDariProfil = (ahliLengkap) => {
  bukaEdit(ahliLengkap);
};

const simpanEdit = async () => {
  saving.value = true;
  try {
    await api.put(`/admin/kemaskini-ahli/${formEdit.value.no_kp}`, { no_ahli: formEdit.value.no_ahli, status_ahli: formEdit.value.status_ahli, role: formEdit.value.role });
    showEdit.value = false;
    await muatAhli();
    
    // Jika modal details sedang dibuka, tutupnya supaya ia di-refresh jika dibuka semula
    if (showDetails.value && ahliDipilih.value?.no_kp === formEdit.value.no_kp) {
      showDetails.value = false;
    }
  } catch (e) { alert(e.response?.data?.message || 'Ralat menyimpan maklumat.'); } 
...
const simpanDaftarPukal = async () => {
  ...
  for (const row of borangDaftar.value) {
    try {
      await api.post('/admin/daftar-ahli', {
        no_kp: row.no_kp, nama_pegawai: row.nama_pegawai, penempatan: row.penempatan, pilihan_potongan: 'Potongan Biro angkasa', yuran_bulanan: 0
      });
      berjaya++;
    } catch (e) { gagal++; }
  }
