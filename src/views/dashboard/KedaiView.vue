    ...
    <div class="grid grid-cols-2 gap-4">
      <div v-for="item in produk" :key="item.id" class="bg-white rounded-[24px] p-4 shadow-sm border border-gray-200/60 flex flex-col justify-between">
        ...
        <div class="mt-4">
          ...
          <button @click="beliItem(item)" :disabled="isProcessing && activeItem === item.id" 
                  class="w-full bg-[#08151D] hover:bg-[#0E303D] text-[#87BCB5] font-black uppercase tracking-widest text-[10px] py-3 rounded-xl transition-all shadow-md flex items-center justify-center gap-1.5 disabled:opacity-70">
...
const beliItem = async (item) => {
  ...
  try {
    const response = await api.post('/bayaran/cipta-bil', {
      jenis_bayaran: 'KEDAI', // <--- Mesti tambah baris ini
      keterangan: `Pembelian Kedai: ${item.nama}`, 
      amaun: item.harga // Backend AKAN memproses amaun ini
    });
